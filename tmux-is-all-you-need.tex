...
shown schematically in Figure~\ref{fig:arch}.
...
\begin{lstlisting}
 ...
 Human (root client) |  attach / detach  .  choose-tree  .  gate     |
              ...
              +------------+                                +------------+
\end{lstlisting}
\caption{The Tmux Orchestration Architecture. An orchestrator (decoder) attends over
a stack of worker panes (encoder) via \code{send-keys}/\code{capture-pane}; workers
may recurse into sub-swarms; the human attaches at the root as a supervisory gate.}
\label{fig:arch}
\end{figure}
...
and subgoal). Each head $i$ attends independently,
\begin{equation}
...
\code{client-attached}, and \code{client-detached} \cite{tmuxhooks}. Activity/silence
...
event the orchestrator must attend to next. This gives the swarm a \emph{positional}
